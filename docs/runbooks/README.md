...
| [Proxmox VM Restore](proxmox-vm-restore.md) | Operational / tested | `dns01` and `proxy01` were restored to isolated temporary VMs, locally validated, and removed. |
...
| [Internal Certificate Lifecycle](internal-certificate-lifecycle.md) | Operational baseline | Root CA creation, wildcard issuance, trust installation, proxy deployment, and validation were exercised; root-key recovery and CA rotation remain untested. |
| [Disaster Recovery](disaster-recovery.md) | Operational baseline | Recovery order is documented and representative `dns01` and `proxy01` whole-VM paths are tested; connected-service and `mon01` recovery remain less mature. |
...
| [VM Provisioning](vm-provisioning.md) | Baseline checklist | Grounded in `dns01`, `mon01`, and `proxy01`; further standardization remains useful. |
| [Maintenance](maintenance.md) | Draft baseline | Includes current Proxmox, monitoring, DNS, authentication, backup, proxy, and rollback checks but is not yet validated as a complete maintenance window. |
